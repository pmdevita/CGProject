...
const vertexShader = `
  ...
  void main() {
    ...
    float posX = (offsetCoords.x + mod((uv2.x * uvScale), uvScale)) * unwrapSlider + (1.f - unwrapSlider) * worldPosition.x;
    float posY = offsetCoords.y * unwrapSlider + (1.f - unwrapSlider) * worldPosition.y;
    float posZ = (offsetCoords.z + mod((uv2.y * uvScale), uvScale)) * unwrapSlider + (1.f - unwrapSlider) * worldPosition.z;
